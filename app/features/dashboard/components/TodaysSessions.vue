<template>
  <div class="bg-(--bg-surface) rounded-[14px] border border-(--border) overflow-hidden">
    <header class="flex items-center justify-between py-[18px] px-[22px] pb-3.5 border-b border-[#F0F4F1] dark:border-white/5">
      ...
      <a href="#" class="text-xs font-semibold text-primary dark:text-(--green-light) cursor-pointer no-underline">View Calendar →</a>
    </header>
    <div>
      <div
        v-for="(s, i) in sessions"
        :key="s.name"
        class="flex items-center gap-3.5 py-3.5 px-[22px] transition-colors duration-100 hover:bg-[#F9FBF9] dark:hover:bg-white/[.03]"
      ...
      >
        ...
        <span
          class="text-[11px] font-semibold py-[3px] px-2.5 rounded-[20px]"
