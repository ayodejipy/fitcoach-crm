<template>
  <div class="bg-(--bg-surface) rounded-[14px] border border-(--border) overflow-hidden">
    <header class="flex items-center justify-between py-[18px] px-[22px] pb-3.5 border-b border-[#F0F4F1] dark:border-white/5">
      ...
      <a href="#" class="text-xs font-semibold text-primary dark:text-(--green-light) cursor-pointer no-underline">View All →</a>
    </header>
    <div>
      <div
        v-for="(c, i) in checkIns"
        :key="c.name + c.time"
        class="flex items-start gap-3 py-3.5 px-[22px] transition-colors duration-100 hover:bg-[#F9FBF9] dark:hover:bg-white/[.03]"
        :class="i < checkIns.length - 1 ? 'border-b border-[#F0F4F1] dark:border-white/5' : ''"
      >
        <div
          class="w-2.5 h-2.5 rounded-full mt-[5px] shrink-0"
          :class="c.unread ? 'bg-[#E74C3C]' : 'bg-[#BDC3C7]'"
        ></div>
        <div class="flex-1">
