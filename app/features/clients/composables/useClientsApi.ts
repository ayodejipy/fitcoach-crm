...
export function useClientsApi() {
  ...
  const list = (params?: { page?: number; per_page?: number; status?: string; search?: string }) =>
    $api<HandlersListClientsResponse>('/api/clients', { params })

  const get = (id: string) =>
    $api<ModelsClient>(`/api/clients/${id}`)

  const invite = (id: string) =>
    $api(`/api/clients/${id}/invite-portal`, { method: 'POST' })

  // --- mutations ---

  const create = (body: HandlersCreateClientRequest) =>
    $api<ModelsClient>('/api/clients', { method: 'POST', body })

  const update = (id: string, body: HandlersUpdateClientRequest) =>
    $api<ModelsClient>(`/api/clients/${id}`, { method: 'PATCH', body })
