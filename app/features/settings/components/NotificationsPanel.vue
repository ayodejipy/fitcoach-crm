...
const { data: settingsData } = useAsyncData('coach-settings', () => settingsApi.getSettings())

watch(settingsData, (s) => {
  if (!s) return
  prefs.checkinNew      = s.notif_checkin_new      ?? true
  prefs.checkinOverdue  = s.notif_checkin_overdue  ?? true
  prefs.checkinStreak   = s.notif_checkin_streak   ?? false
  prefs.paymentReceived = s.notif_payment_received ?? true
  prefs.paymentFailed   = s.notif_payment_failed   ?? true
  prefs.scheduleChange  = s.notif_schedule_change  ?? true
  prefs.scheduleDigest  = s.notif_schedule_digest  ?? false
}, { immediate: true })
...
async function save() {
  ...
  try {
    await settingsApi.updateSettings({
      notif_checkin_new:      prefs.checkinNew,
      notif_checkin_overdue:  prefs.checkinOverdue,
      notif_checkin_streak:   prefs.checkinStreak,
      notif_payment_received: prefs.paymentReceived,
      notif_payment_failed:   prefs.paymentFailed,
      notif_schedule_change:  prefs.scheduleChange,
      notif_schedule_digest:  prefs.scheduleDigest,
    })
...
<template>
  <div>
    ...
    <SettingsCard title="Schedule">
      ...
      <template #footer>
        <UButton color="primary" size="lg" :loading="saving" @click="save">
          Save Preferences
        </UButton>
      </template>
    </SettingsCard>
