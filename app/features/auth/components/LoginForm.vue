  ...
  <div>
    ...
    <UForm :schema="schema" :state="state" class="flex flex-col gap-4" @submit="handleSubmit">
      ...
      <UFormField name="password">
        <template #label>
          <span class="text-[13px] font-semibold text-(--text-secondary) flex items-center justify-between w-full">
            Password
            <button
              type="button"
    ...
    <div class="mt-5 text-center text-[13px] text-(--text-muted)">
      ...
      <a href="#" class="text-(--text-accent) font-semibold no-underline" @click.prevent="$emit('switch', 'signup')">Create one free →</a>
    </div>
  </div>
</template>
